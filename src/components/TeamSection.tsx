...
export default function TeamSection({ projects, initialTeamMembers = [], onUpdateTeamMembers, onUpdateProjects }: TeamSectionProps) {
  // Team members state
  ...
  const addTeamMember = () => {
    ...
    if (onUpdateProjects && newMember.selectedProjects.length > 0) {
    ...
    newMember.selectedProjects.forEach(projectId => {
        ...
        const assignedTeam = [...(project.assignedTeam || []), member.id];
          // Update the project with the new assignedTeam array
      ...
      // Call the callback to update projects in parent component
  ...
  const updateTeamMember = (id: string) => {
    ...
    // Update project assignments if we have the callback
    if (onUpdateProjects) {
      const updatedProjects = [...projects];
      const currentMemberProjects = getMemberProjects(id).map(p => p.id);
      
      // Remove member from projects that are not selected anymore
      updatedProjects.forEach((project, index) => {
        if (project.assignedTeam?.includes(id) && !editForm.selectedProjects.includes(project.id)) {
          updatedProjects[index] = {
            ...project,
            assignedTeam: project.assignedTeam.filter(memberId => memberId !== id)
          };
        }
      });
      
      // Add member to newly selected projects
      editForm.selectedProjects.forEach(projectId => {
        if (!currentMemberProjects.includes(projectId)) {
          const projectIndex = updatedProjects.findIndex(p => p.id === projectId);
          if (projectIndex !== -1) {
            const project = updatedProjects[projectIndex];
            const assignedTeam = [...(project.assignedTeam || []), id];
            updatedProjects[projectIndex] = {
              ...project,
              assignedTeam
            };
  ...
  const deleteTeamMember = (id: string) => {
    ...
    // Remove team member from all projects
    if (onUpdateProjects) {
      const updatedProjects = projects.map(project => {
        if (project.assignedTeam?.includes(id)) {
          return {
            ...project,
            assignedTeam: project.assignedTeam.filter(memberId => memberId !== id)
          };
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>, isEdit = false) => {
    ...
    reader.onload = (e) => {
      ...
      img.onload = () => {
        ...
        const resizedImage = canvas.toDataURL(file.type, 0.7); // 0.7 quality to reduce size
        
        // Set the URL in the form
      if (isEdit) {
          setUploadedEditImage(resizedImage);
          setEditForm(prev => ({ ...prev, avatar: resizedImage }));
      } else {
          setUploadedImage(resizedImage);
          setNewMember(prev => ({ ...prev, avatar: resizedImage }));
      }
      };
  ...
  return (
    <div className="space-y-6 animate-fade-in" style={{ paddingBottom: "100px", height: "auto", maxHeight: "calc(100vh - 50px)", overflow: "visible" }}>
      {/* Tab Navigation */}
      ...
      {activeTab === 'members' && (
          ...
          {/* Team Member List */}
            <div className="space-y-4 overflow-y-auto" style={{ maxHeight: showFilters ? 'calc(100vh - 280px)' : 'calc(100vh - 160px)', paddingBottom: "200px" }}>
      ...
      {activeTab === 'projects' && (
          ...
          <h3 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Project Teams</h3>
            ...
            <div className="space-y-6 overflow-y-auto" style={{ maxHeight: 'calc(100vh - 160px)', paddingBottom: "200px" }}>
            {projects.map(project => (
